...
namespace Celeste.Mod.ExCameraDynamics.Code.Hooks
{
    public static partial class CameraZoomHooks
    {
        ...
        public static void ResizeVanillaBuffers(float zoomTarget)
        {
            ...
            // Find the dimensions
            BufferWidthOverride = (current_buffer_zoom >= 1f ? 320 : (int)Math.Min(MaxBufferWidth, Math.Ceiling(320f / current_buffer_zoom)));
            BufferHeightOverride = (current_buffer_zoom >= 1f ? 180 : (int)Math.Min(MaxBufferHeight, Math.Ceiling(180f / current_buffer_zoom)));
